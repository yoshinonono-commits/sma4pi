...
2. 出てきたダイアログで X 軸・Y 軸にとる列を指定する(X・Y 誤差列も指定可)
...
プリセット(直線・2次・指数・ガウス・ローレンツ・Voigt・誤差関数(erf) など)から選ぶか、自分で式を書きます。

- パラメータ名をカンマ区切りで入れると、初期値の入力欄が自動で並びます
- 実行すると収束値・標準誤差・χ²・R² が出ます
- 収束値は初期値欄に書き戻されるので、そのまま追い込めます
- 誤差列を読み込んでいれば重み付きフィットもできます
- **フィット範囲を指定する** を on にすると、指定した x 区間のデータだけを使ってフィットできます
  (グラフに追加されるフィット曲線も、その区間だけに描かれます)
...
分光データなど、複数のピークが重なったスペクトルを1本の式でまとめてフィットしたい
ときに使います。「多重ピーク」欄でピーク形状(ガウス/ローレンツ/Voigt)とピーク数を
選び、**この形状で式を生成** を押すと式・パラメータ・初期値がまとめて作られます。
ピーク中心の初期値はデータの x 範囲に等間隔で仮置きされるので、実際のピーク位置に
近づけてから実行してください。

### 残差プロット

**グラフの設定** → **残差プロットを表示する** を on にすると、フィット曲線に対応する
系列がある場合、グラフの下に「データ − モデル」を並べて表示します。フィットの
当てはまりの良し悪しが一目で分かります。フィット元の系列が消えるなど対応が
取れなくなった場合は自動的に非表示になります。
...
X・Y・X誤差・Y誤差を表形式で直接書き換えられます。行の追加・削除もここで行います。
...
グラフは `.s4p` (JSON) で保存され、既定ではデータも一緒に埋め込まれます。
保存形式は version 2 ですが、version 1 のファイルもそのまま開けます。

**ファイル** → **データを埋め込んで保存** のチェックを外すと、テキストファイルから
読み込んだ系列はデータを埋め込まず、開くときに元ファイルから読み直すようになります
(`.s4p` を軽くしたいときに便利)。元ファイルが見つからないと、その系列だけ
警告付きで空になります。手作業でデータ点を編集した系列は、元ファイルの行と対応が
取れなくなるため常に埋め込まれます。
